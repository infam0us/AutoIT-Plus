...
void AutoIt_Script::Parser_Keyword_IF(VectorToken &vLineToks, uint &ivPos, int &nScriptLine)
{
	...
	if (vLineToks[ivPos].m_nType == TOK_KEYWORD && vLineToks[ivPos].nValue == K_ISE)
		...
		if (vIFToks[ivTempPos].m_nType == TOK_KEYWORD)
		{
			switch (vIFToks[ivTempPos].nValue)
			{
				case K_EGER:
					...
					{
						if (vIFToks[ivTempPos].m_nType == TOK_KEYWORD && vIFToks[ivTempPos].nValue == K_ISE)
						{
...
AUT_RESULT AutoIt_Script::Parser_VerifyBlockStructure(void)
{
	...
	while ( (szScriptLine = g_oScriptFile.GetLine(nScriptLine)) != NULL )
	{
		...
		switch (LineTokens[0].nValue)
		{
			...
			case K_EGER:							// Only take notice of multiline IFs
				...
				{
					if (LineTokens[ivPos].m_nType == TOK_KEYWORD && LineTokens[ivPos].nValue == K_ISE)
					{
						bThen = true;
				...
				if (LineTokens[ivLast].m_nType == TOK_KEYWORD && LineTokens[ivLast].nValue == K_ISE)
				{
					++nIf;
					stkIf.push(nSeq++);
				}
